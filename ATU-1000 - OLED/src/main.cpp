...
// =============================================================================
// Display Manager - 2.42" 128x64 OLED (SSD1309) with U8g2
// =============================================================================

class DisplayManager {
 public:
  // HW_I2C constructor matching working example: (rotation, reset, clock, data)
  DisplayManager() : u8g2_(U8G2_R0, /* reset=*/ U8X8_PIN_NONE, /* clock=*/ 21, /* data=*/ 47) {}
  ...
  void begin() {
    ...
    Serial.printf("  SSD1309 NONAME2 at 0x%02X\n", kCfg.oledAddress);
    Serial.println("  I2C pins: SCL=GPIO21, SDA=GPIO47");

    delay(100);  // Power-up delay

    // Set I2C address (U8g2 uses 8-bit address)
    u8g2_.setI2CAddress(kCfg.oledAddress * 2);

    // Initialize - U8g2 handles Wire internally
    u8g2_.begin();
  ...
  void showSplash(const char* version) {
    ...
    u8g2_.drawStr(20, 20, "ATU-1000");

    // Subtitle
    u8g2_.setFont(u8g2_font_helvR08_tr);
    u8g2_.drawStr(15, 35, "ESP32-S3 Controller");

    // Decorative line
    u8g2_.drawHLine(10, 42, 106);

    // Version and status
    u8g2_.setFont(u8g2_font_6x10_tr);
    u8g2_.drawStr(4, 55, version);
    u8g2_.drawStr(40, 55, "Starting...");
  ...
  void render(uint32_t freqHz, bool catConnected, bool autoTuneEnabled,
    ...
    if (freqHz > 0) {
      ...
      uint32_t khz = (freqHz / 1000UL) % 1000UL;
      snprintf(freqBuf, sizeof(freqBuf), "%2lu.%03lu", (unsigned long)mhz, (unsigned long)khz);
    } else {
      snprintf(freqBuf, sizeof(freqBuf), "--.---");
    }
    u8g2_.drawStr(0, 22, freqBuf);

    // MHz label
    u8g2_.setFont(u8g2_font_6x10_tr);
    u8g2_.drawStr(85, 22, "MHz");

    // ========== SWR Bar + Value (y=26-34) ==========
    const int swrBarY = 26;
    const int swrBarW = 80;
    u8g2_.drawFrame(0, swrBarY, swrBarW, 8);

    // SWR bar fill (1.0 = empty, 3.0 = full)
    float swrPct = constrain((reading.swr - 1.0f) / 2.0f, 0.0f, 1.0f);
    int swrFill = static_cast<int>(swrPct * (swrBarW - 2));
    if (swrFill > 0) {
      u8g2_.drawBox(1, swrBarY + 1, swrFill, 6);
    }

    // SWR value to right of bar
    u8g2_.setFont(u8g2_font_helvB10_tr);
    char swrBuf[8];
    if (reading.swr < 10.0f) {
      snprintf(swrBuf, sizeof(swrBuf), "%.1f", reading.swr);
    } else {
      snprintf(swrBuf, sizeof(swrBuf), ">10");
    }
    u8g2_.drawStr(84, 34, swrBuf);

    // ========== Power Bar + Value (y=38-46) ==========
    const int pwrBarY = 38;
    const int pwrBarW = 80;
    u8g2_.drawFrame(0, pwrBarY, pwrBarW, 8);

    float pct = constrain(reading.powerW / kPowerLimitW, 0.0f, 1.0f);
    int barWidth = static_cast<int>(pct * (pwrBarW - 2));
    if (barWidth > 0) {
      if (powerOverload || powerWarning) {
        for (int x = 1; x < barWidth + 1; x += 2) {
          u8g2_.drawVLine(x, pwrBarY + 1, 6);
        }
      } else {
        u8g2_.drawBox(1, pwrBarY + 1, barWidth, 6);
      }
    }

    // Power value to right of bar
    u8g2_.setFont(u8g2_font_helvB10_tr);
    char pwrBuf[10];
    snprintf(pwrBuf, sizeof(pwrBuf), "%dW", (int)reading.powerW);
    u8g2_.drawStr(84, 46, pwrBuf);

    // ========== Bottom Status Row ==========
    u8g2_.drawHLine(0, 50, 112);
    u8g2_.setFont(u8g2_font_6x10_tr);
    ...
    // CAT status
    if (catConnected) {
      u8g2_.drawRBox(xPos, 53, 22, 11, 2);
      u8g2_.setDrawColor(0);
      u8g2_.drawStr(xPos + 3, 62, "CAT");
    ...
    // Auto/Manual
    if (autoTuneEnabled) {
      u8g2_.drawRBox(xPos, 53, 28, 11, 2);
      u8g2_.setDrawColor(0);
      u8g2_.drawStr(xPos + 2, 62, "AUTO");
      u8g2_.setDrawColor(1);
    } else {
      u8g2_.drawStr(xPos + 2, 62, "MAN");
    }
    xPos += 32;

    // Hi-Z / Lo-Z
    u8g2_.drawStr(xPos, 62, state.topology ? "HiZ" : "LoZ");
    xPos += 24;
    ...
    if (state.bypass) {
      u8g2_.drawRBox(xPos, 53, 22, 11, 2);
      u8g2_.setDrawColor(0);
      u8g2_.drawStr(xPos + 2, 62, "BYP");
      u8g2_.setDrawColor(1);
    }

    // ========== OVERLOAD OVERLAY ==========
    if (powerOverload) {
      u8g2_.drawFrame(4, 15, 104, 30);
      u8g2_.setDrawColor(0);
      u8g2_.drawBox(5, 16, 102, 28);
      u8g2_.setDrawColor(1);
      u8g2_.setFont(u8g2_font_helvB12_tr);
      u8g2_.drawStr(8, 36, "! OVERLOAD !");
    }
  ...
  void showSweepGraph(const std::vector<SweepPoint>& points, uint32_t startHz, uint32_t endHz) {
    ...
    // Title bar
    u8g2_.setFont(u8g2_font_6x10_tr);
    u8g2_.drawStr(0, 8, "SWR SWEEP");

    // Find min SWR for display
    size_t minIdx = 0;
    float minSwr = 99.0f;
    for (size_t i = 0; i < points.size(); ++i) {
      if (points[i].swr < minSwr) {
        minSwr = points[i].swr;
        minIdx = i;
      }
    }

    // Graph area - fit within 120px width
    ...
    // Draw axes
    u8g2_.drawHLine(graphLeft, graphBottom, graphWidth);
    u8g2_.drawVLine(graphLeft, graphTop, graphHeight);

    // Y-axis labels
    u8g2_.setFont(u8g2_font_4x6_tr);
    u8g2_.drawStr(0, graphTop + 4, "5");
    u8g2_.drawStr(0, graphBottom, "1");

    // Plot SWR curve
    for (size_t i = 0; i < points.size(); ++i) {
      int x = graphLeft + (i * graphWidth) / points.size();
      float swr = std::min(points[i].swr, 5.0f);
      int y = graphBottom - static_cast<int>((swr - 1.0f) / 4.0f * graphHeight);
      y = constrain(y, graphTop, graphBottom);
      u8g2_.drawVLine(x, y, graphBottom - y);
    }

    // Mark minimum
    int minX = graphLeft + (minIdx * graphWidth) / points.size();
    int minY = graphBottom - static_cast<int>((minSwr - 1.0f) / 4.0f * graphHeight);
    minY = constrain(minY, graphTop, graphBottom);
    u8g2_.drawCircle(minX, minY, 3);

    // Bottom info
    u8g2_.setFont(u8g2_font_5x7_tr);
    char infoBuf[24];
    snprintf(infoBuf, sizeof(infoBuf), "%.2f@%.2fM", minSwr, points[minIdx].freqHz / 1e6);
    u8g2_.drawStr(0, 63, infoBuf);
  ...
  void showOverloadWarning() {
    ...
    // Inverted display for maximum visibility
    u8g2_.drawBox(0, 0, 126, 64);
    u8g2_.setDrawColor(0);

    // Warning text
    u8g2_.setFont(u8g2_font_helvB14_tr);
    u8g2_.drawStr(10, 28, "! OVERLOAD !");

    u8g2_.setFont(u8g2_font_helvR10_tr);
    u8g2_.drawStr(10, 48, "BYPASS ENGAGED");

    u8g2_.setDrawColor(1);
    u8g2_.sendBuffer();
  }

  void showTuning(uint8_t progress) {
    if (!enabled_) return;
    u8g2_.clearBuffer();

    u8g2_.setFont(u8g2_font_helvB14_tr);
    u8g2_.drawStr(25, 22, "TUNING");

    // Progress bar
    u8g2_.drawFrame(8, 32, 110, 12);
    int barW = (progress * 106) / 100;
    if (barW > 0) {
      u8g2_.drawBox(10, 34, barW, 8);
    }
    ...
    u8g2_.drawStr(50, 58, pctBuf);

    u8g2_.sendBuffer();
  }

 private:
  // 2.42" SSD1309 128x64 OLED - confirmed working constructor
...
void handleCommand(const char* line) {
  ...
  if (strcasecmp(line, "i2cscan") == 0) {
    ...
    if (found == 0) {
      ...
      Serial.println("  Check: pull-up resistors (4.7k), wiring, VCC");
    } else {
